    ...
    mifosX.controllers = _.extend(module, {
        MainController: function (scope, location, sessionManager, translate, $rootScope, localStorageService, keyboardManager, $idle, tmhDynamicLocale,
            ...
            var recentClient = {};
        	 ...
        	 if (localStorageService.getFromLocalStorage('recentClients')) {
        		 recentClientArray = localStorageService.getFromLocalStorage('recentClients');
             }
             scope.$watch(function () {
           	  ...
           	  if(scope.clientAccountNo){
             		...
             		localStorageService.addToLocalStorage('recentClients', recentClientArray);
           	  	}
             });
            ...
            scope.isActive = function (route) {
                if (route == 'clients') {
                    ...
                    var temp1 = ['/system', '/users/', '/admin/roles', 'admin/viewmctasks', '/audit', '/global', '/mappingconfig', '/datatables', '/reports', '/codes',
                                 '/organization', '/addressmanage', '/offices', '/employees', '/property', '/groupsDetails', '/message', '/broadcasterchannelmapping', 
                                 '/contract', '/chargecode', '/discounts', '/regions', '/services', '/plans', '/mediadetailseventpricing', 
                                 '/accounting', '/freqposting', '/accounting_coa', '/journalentry', '/accounts_closure', '/Searchtransaction', '/accounting_rules',
                                 '/importing', '/jobs', '/paymentGateway', '/eventaction'];
                    for (var i in temp1) {
                    ...
                    var temp2 = ['/reports/all', '/reports/clients', '/reports/Orders', '/reports/tickets', '/reports/inventory', 'reports/leads', 'reports/invoice&collections'];
                    for (var i in temp2) {
                ...
                /*else if (route == 'admin') {
                    var temp3 = ['/system', '/organization', '/system', '/accounting', '/importing', '/jobs', '/paymentGateway', '/eventaction'];
                    for (var i in temp3) {
                ...
                else if (route == 'assignedtickets') {
                    var temp6 = ['/assignedtickets'];
                    for (var i in temp6) {
                        if (temp6[i] == location.path()) {
                            return true;
                        }
                    }
                } else if (route == 'referal') {
                    var temp7 = ['/referal'];
                    for (var i in temp7) {
                        if (temp7[i] == location.path()) {
                            return true;
                        }
                    }
                }
            ...
            $rootScope.register = function(){
                $rootScope.isRegister = !$rootScope.isRegister;
            }
